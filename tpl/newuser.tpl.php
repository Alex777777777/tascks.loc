<?php
    ...
    if(($user->role!=1)&($user->id!=$_GET['item'])){
    ...
    if(isset($_GET['item'])){
        ...
        $caps="Просмотр пользователя ".$nuser->name;
    }
    
?>
<div class="sd_hd"><?php echo $caps;?></div>
<form autocomplete="off" data-id="<?php echo $nuser->id;?>">
<div class="frm_str"><label for="usr_name">Логин</label><input id="usr_name" autocomplete="off" type="text" placeholder="логин" required value="<?php echo $nuser->name;?>" <?php  if($nuser->id) echo " readonly";?>></div>
...
<?php }else{
    ...
    echo "<span class='usr_roles'>$stype</span>";
}    
?>
<?php
